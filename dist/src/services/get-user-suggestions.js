 ...
 */
export const getUserSuggestions = async ({ userId, limit = 5 }) => {
    try {
        // Primeiro, verifica se o usuário tem conexões no Neo4j
        const connectionCheckResult = await driver.executeQuery(`MATCH (u:User {id: $userId})
             OPTIONAL MATCH (u)-[:FOLLOWS]->(following)
             RETURN count(following) as followingCount`, { userId });
 ...
 */
async function getSmartSuggestions(userId, limit) {
    try {
